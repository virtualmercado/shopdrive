...
-- Fix the sync_template_from_profile function - products table doesn't have a category column
...
BEGIN
  ...
  -- Insert products - products table has category_id (FK), not category text field
  -- Set category to 'Geral' as default since we can't easily get the category name
  ...
  SELECT 
    ...
    p.price,
    p.images,
    'Geral', -- Default category since products uses category_id FK
    NULL,
    true,
