...
-- Fix the sync_template_from_profile function to not use non-existent is_active column
...
BEGIN
  ...
  -- Sync products count (count all products for this profile)
  UPDATE brand_templates SET
    products_count = (
  ...
  -- Insert products without referencing is_active (which doesn't exist in products table)
  ...
  SELECT 
    ...
    p.price,
    p.images,
    COALESCE((SELECT c.name FROM categories c WHERE c.id = p.category_id), 'Geral'),
    NULL, -- sku doesn't exist in products table
