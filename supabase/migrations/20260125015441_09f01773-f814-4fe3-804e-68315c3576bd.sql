...
-- Fix the sync_template_from_profile function to not reference non-existent categories table
...
BEGIN
  ...
  -- Insert products - use category directly from products table (it's a text field, not a foreign key)
  ...
  SELECT 
    ...
    p.price,
    p.images,
    COALESCE(p.category, 'Geral'), -- category is a text field directly on products
    NULL,
    true,
