...
-- Immutable normalization helper (unaccent is stable, wrap in immutable SQL)
...
AS $$
  SELECT trim(
    regexp_replace(
      lower(public.unaccent('public.unaccent', coalesce(input_text, ''))),
      '[^a-z0-9]+',
      ' ',
      'g'
    )
...
-- Trigram index on normalized product search text (name + category + brand)
CREATE INDEX IF NOT EXISTS products_search_norm_trgm_idx
  ON public.products
  USING gin (
...
-- Composite index used by the RPC filter
...
BEGIN
  ...
  filtered AS (
    SELECT * FROM scored
    WHERE length(v_q) = 0 OR relevance > 30
  ),
  counted AS (
  ...
  FROM filtered f
  ORDER BY f.relevance DESC NULLS LAST,
