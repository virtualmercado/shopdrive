...
import { Loader2, Search, Play, RefreshCw, AlertTriangle, CheckCircle2, XCircle } from 'lucide-react';
...
const TemplateMaintenanceTab = () => {
  ...
  // Fetch pending/failed stores
  const { data: pendingStores, isLoading, refetch } = useQuery({
    queryKey: ['pending-template-stores', selectedTemplateId, searchEmail],
    queryFn: async () => {
      let query = supabase
        .from('profiles')
        .select('id, store_name, full_name, source_template_id, template_applied, template_apply_status, template_apply_error, template_applied_at, created_at')
  ...
  const handleRunBackfill = async () => {
    ...
    try {
      ...
      toast.error('Erro inesperado no backfill');
    } finally {
  ...
  const getStatusBadge = (store: PendingStore) => {
    if (store.template_applied && store.template_apply_status === 'applied') {
      return <Badge className="bg-green-100 text-green-800"><CheckCircle2 className="h-3 w-3 mr-1" /> Aplicado</Badge>;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="text-sm text-muted-foreground">Pendentes / Falhas</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <div className="text-2xl font-bold text-green-600">{appliedCount}</div>
            <p className="text-sm text-muted-foreground">Templates aplicados</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Aplicar templates retroativamente em contas pendentes (modo seguro — não sobrescreve dados existentes)
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center gap-4">
            <Button
              ...
              disabled={isRunningBackfill || pendingCount === 0}
            >
              {isRunningBackfill ? (
                <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Processando...</>
              ) : (
                <><Play className="h-4 w-4 mr-2" /> Executar Backfill ({pendingCount} pendentes)</>
              )}
            </Button>
        ...
        <CardContent className="space-y-4">
          ...
          {forceMode && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                Modo forçado ativado: a reaplicação substituirá produtos e configurações existentes.
              </AlertDescription>
            </Alert>
          )}
                  ...
                  {(!pendingStores || pendingStores.length === 0) ? (
                    <TableRow>
                      <TableCell colSpan={6} className="text-center text-muted-foreground py-8">
                        Nenhuma loja encontrada com template vinculado
                      </TableCell>
                    </TableRow>
                  ) : (
                        ...
                        <TableCell className="text-right">
                          {store.template_applied && store.template_apply_status === 'applied' && !forceMode ? (
                            ...
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => handleApplyToStore(store.id, store.source_template_id)}
                              disabled={applyingStoreId === store.id}
                            >
                              {applyingStoreId === store.id ? (
                                <Loader2 className="h-3 w-3 animate-spin" />
                              ) : (
                                <>{forceMode ? 'Forçar' : 'Aplicar'}</>
                              )}
                            </Button>
                          )}
                        </TableCell>
